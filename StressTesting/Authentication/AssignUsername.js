...
export default function () {
  group("Verify assigning username", function(){
    const url = 'http://backend.gigachat.cloudns.org/api/user/AssignUsername/';
  
  const payload = JSON.stringify({
    username: 'mahmoud_ossama'
  });
...
export function handleSummary(data) {
  return {
    ...
    "AssignUsernameReport.json": JSON.stringify(data), //the default data object
  };
} 
  //sleep(1);