describe('Checking login credentials',function()
{
    ...
    */
    ...
    {
        cy.visit('https://www.gigachat.cloudns.org/');
        cy.xpath("//body/div[@id='root']/div[1]/div[1]/div[3]").should('not.exist');
        cy.xpath("//div[contains(text(),'Sign in')]").should('exist').click();
        cy.wait(5000);
        cy.xpath("//body/div[@id='root']/div[1]/div[1]/div[3]").should('be.visible');
        ...
        cy.wait(5000);
        cy.xpath("//div[contains(text(),'Request failed with status code 404')]").should('exist');
    }
    )
    ...
    it('Verify that the password can be seen or not seen',function()
    {
        ...
        cy.get('#secondPage > div > form > div.relative > span').should('exist').click();
    ...
    it('Verify that the secondary email in the forgotten password is written and the mail will be forwarded to it',function()
    {
        cy.visit('https://www.gigachat.cloudns.org/');
        cy.xpath("//body/div[@id='root']/div[1]/div[1]/div[3]").should('not.exist');
        cy.xpath("//div[contains(text(),'Sign in')]").should('exist').click();
        ...
        cy.get('#page3 > div').should('contain','mahmoud.khattab13@gmail.com');
    }
    )
}
)